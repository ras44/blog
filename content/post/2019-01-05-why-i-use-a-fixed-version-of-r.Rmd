...
* I open the project in RStudio and begin work.  All packages are pre-built, so I don't have to go through lengthy installs.  I can upgrade packages in the `Packrat Library` of the "Packages" tab, and then run `packrat::snapshot()` to save the new libraries into my project's `packrat/` directory.
* If I ever need to duplicate results, I can always build the same fixed-R instance, clone the project I created, and know that it will work exactly the same as when I previously worked on it... sometimes years earlier.
